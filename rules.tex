...
% \begin{itemize}
%   \item \textbf{Définir les paramètres :} Le nombre de jouer, la taille des squads au sein des 2 grandes équipes et le nombre de trésor à trouver par squad \textit{(On va partir sur un exemple de 40 joueurs, des squads de 4 joueurs et 2 trésors à trouver par squad)}.
%   \item \textbf{Définir les types et valeurs des trésors :} \textit{On va prendre l'exemple de pièce de valeur 1, gemme de valeur 2, 2 statuettes et un masque en or}.
%   \item \textbf{Définir la répartition bourse / coffre :} \textit{Dans notre exemple, $\frac{2}{3}$ de bourse et $\frac{1}{3}$ de coffre}.
%   \item \textbf{Définir le nombre de trésors par conteneur :} \textit{3 pièces et 1 gemme en moyenne par bourse et 5 pièces et 2 gemmes en moyenne par coffre}.
%   \item \textbf{\textit{La partie calcul peut maintenant commencer}}
%   \item \textbf{Calculer le nombre de trésor total à trouver :}
%   \begin{itemize}
%     \item On calcul le nombre total de petite squad : Le nombre total de joueur divisé par la taille des squads \textit{($40 \div 4 = 10$)}.
%     \item On calcul le nombre de trésor : Le nombre de squad multiplié par le nombre de trésor à trouver par squad \textit{($10 \cdot 2 = 20$)}.
%   \end{itemize}
%   \item \textbf{Calculer le nombre de coffre et bourse :} On enlèvement le nombre de gros trésor (statuette et masque en or) au nombre total de trésors \textit{($20 - 3 = 17$)}.
%   \item \textbf{Calculer le nombre de pièces et gemmes :}
%   \begin{itemize}
%     \item Calculer le nombre exact de coffre en multipliant le nombre total des 2 par la répartition coffre/bourse : \textit{($\frac{1}{3} \cdot 17 = 6$)}.
%     \item Calculer le nombre exact de bourse de la même manière \textit{($\frac{2}{3} \cdot 17 = 11$)}.
%     \item Calculer le nombre de pièces : on addition le nombre de pièces des coffres avec ceux des bourse \textit{($6 \cdot 5 + 11 \cdot 3 = 63$)}.
%     \item Calculer le nombre de gemmes : Idem que pour les pièces \textit{($6 \cdot 2 + 11 \cdot 1 = 23$)}. 
%   \end{itemize}
%   \item \textbf{Calculer la valeur des autres trésors :}
%   \begin{itemize}
%     \item Calculer le nombre de point des pièces et gemmes \textit{($63 \cdot 1 + 23 \timcdotes 2 = 109$)}.
%     \item Prendre environ la moitié de cette valeur pour valeur des autres trésors \textit{($109 \div 2 = 54$)}.
%     \item Appliquer un coefficient de rareté \textit{(2 statuettes coef 1 et 1 masque coef 1.5)}.
%     \item Calculer le nombre de trésor en appliquant ce coefficient \textit{($2 \cdot 1 + 1 \cdot 1.5 = 3.5$)}.
%     \item Calculer la valeur des trésors restants \textit{(Statuette : $1 \cdot 54 \div 3.5 = 15$, Masque : $1.5 \cdot 54 \div 3.5 = 23$)}.
%   \end{itemize}
%   \item \textbf{Répartir les pièces et les gemmes :} Les répartir dans les coffres et bourses selon ce qui a été définie plus tôt.
% \end{itemize}

% Voir en annexe plusieurs exemples de calcul selon les paramètres d'entrés.
